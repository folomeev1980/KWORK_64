...
def excel_creator(dic_list, new=True):
    ...
    checker = int(sheet_ki.cell(row=2, column=19).value)
    checker2 = int(sheet_ki.cell(row=2, column=2).value)
    for liner, i in enumerate(range(2, len(sheet_ki['A']) + 1)):
        if int(sheet_ki.cell(row=i, column=19).value) == checker and int(sheet_ki.cell(row=i, column=2).value) == checker2:
            pass
        else:
            ...
            for col in range(1, 18):
                sheet_ki.merge_cells(start_row=i - checker, start_column=col, end_row=i - 1,
                                     end_column=col)
            checker = int(sheet_ki.cell(row=i, column=19).value)
            checker2 = int(sheet_ki.cell(row=i, column=2).value)
