...
def writer_csv_include(dic):
    if 'email' not in dic:
        dic['email'] = ""
    # if 'Электронная почта' not in dic:
    #     dic['Электронная почта'] = ""
    # if 'Телефон' not in dic:
    #     dic['Телефон'] = ""
    # if 'Сведения о руководителе' not in dic:
    #     dic['Сведения о руководителе'] = ""
    # if 'Адрес местонахождения юридического лица' not in dic:
    #     dic['Адрес местонахождения юридического лица'] = ""
    # if 'Сокращенное наименование' not in dic:
    #     dic['Сокращенное наименование'] = ""

    try:
        with open("sozaso.csv", "a", newline='') as f:
...
def get_info(html):
    ...
    try:
        ...
        soup = soup.find("div", id="anons_text_view").find("div").text
        soup = soup.split("\n")
        for i in soup:
            if ":" in i:
...
if __name__ == "__main__":
    ...
    for i in range(1, 1000000):
        print(i)
        html = get_html_("http://sozaso.ru/firm_name.php?id={}".format(str(i)))
