...
def vk_parser(format_,direction, group_id_list, date_start, date_end, comments, likes, ads, token, list_of):
    mdir(direction)
...
def vk_post(format_, path_folder, min_shift, token_post, owner_id, captcha_sid_, captcha_key_):
    ...
    if time_zero == None:
        ...
        while len(list_files_folder(path_folder, format_)) > 0:
            ...
            poster_result = poster(token_post, owner_id, path_folder, time_of_post, format_, captcha_sid_,
                                   captcha_key_)

            if poster_result[0] != None:
                number_of_posts = get_posts_number(token_post, owner_id)
                print("\n\npost uploaded {}, number of posts <<< {} >>>\n".format(i, number_of_posts),
                      poster_result[0:1],
                ...
                elif "214. Access to adding post denied: can only schedule 25 posts on a day" in str(error):

                    catcha25_timer = timer(datetime.datetime.fromtimestamp(take_time(token_post, owner_id)))
                ...
                elif "Access to adding post denied: cannot schedule more than 150 posts." in str(error):
...
if __name__ == "__main__":
    ...
    if a == "0":
        ...
        vk_parser(format_,direction, group_id_list, date_start, date_end, comments, likes, ads, token, list_of)
        input("\nParsing was done")

    elif a == "1":

        vk_post(format_, path_folder, min_shift, token_post, owner_id, captcha_sid_, captcha_key_)
