...
def writer_csv_include(dic):
    ...
    if 'Полное наименование организации' not in dic:
        dic['Полное наименование организации'] = ""


    try:
        with open("uralsro.csv", "a", newline='') as f:
            writer = csv.writer(f, delimiter=';')
            t=( dic['Полное наименование организации'],
                dic['ИНН'],
                dic['Сведения о руководителе'],
                dic['Юридический адрес'],
                dic['Веб-сайт организации'],
...
if __name__=="__main__":
    ...
    for i in range(1,100000):
        print(i)
        html=get_html_("http://reestr.uralsro.ru/view?id={}".format(str(i)))
        writer_csv_include(get_info(html))
    # html = get_html_("http://reestr.uralsro.ru/view?id=227")
    # print(get_info(html))